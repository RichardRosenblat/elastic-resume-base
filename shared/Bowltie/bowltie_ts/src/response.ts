 ...
 * ```
 */
export function formatSuccess<T>(data: T, correlationId?: string): SuccessResponse<T> {
  return {
    success: true,
    data,
    meta: {
 ...
 * ```
 */
export function formatError(code: string, message: string, correlationId?: string): ErrorResponse {
  return {
    success: false,
    error: { code, message },
    meta: {
