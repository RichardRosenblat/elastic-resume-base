...
let _publisher: IPubSubPublisher | undefined;
 ...
 * ```
 */
export function initializePubSub(projectId: string): void {
 ...
 * ```
 */
export function initializePubSubFromEnv(): void {
  if (_publisher) return;
 ...
 * ```
 */
export function getPublisher(): IPubSubPublisher {
  if (!_publisher) {
    throw new Error(
