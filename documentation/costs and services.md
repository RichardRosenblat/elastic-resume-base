...
| **Search Base (FAISS)** | Cloud Run + Cloud Storage | 5 GB Regional Storage free | **$0.00** (Scaled to zero) |
...
| **DLQ Notifier** | Cloud Run + Pub/Sub | 10 GB Pub/Sub messages/month free | **$0.00** |
| **Persistence (Synapse)** | Firestore | 50k reads, 20k writes, 20k deletes/day | **$0.00** |
...
1. **The Translation Threshold:** We receive 500,000 translated characters for free monthly. Because I implemented the "Synapse" translation cache, we are unlikely to hit this limit at standard volumes. Once we exceed the cache's capacity at higher scales, standard pricing applies ($20 per 1 million characters).
...
3. **The Search Base Warm-Up:** Once our FAISS index exceeds the optimal size for a fast cold start (projected around the 50,000 resume mark), I will configure the Search Base to `min-instances: 1`. This introduces a flat ~$5 to $15 monthly cost to ensure immediate query response times.
...
| **Compute Layers** | Traffic slightly exceeds 180,000 free vCPU seconds. | **~$2.00** |
| **A.I. Extractor** | ~15M input / 5M output tokens processed. | **~$2.60** |
| **A.I. Embedder** | ~10M tokens processed. | **~$1.00** |
...
| **Compute Layers** | Sustained daily container usage. | **~$25.00** |
| **A.I. Extractor** | ~150M input / 50M output tokens processed. | **~$26.00** |
| **A.I. Embedder** | ~100M tokens processed. | **~$10.00** |
...
**Total Estimated Cost: ~$174.50 / month**
...
* **Translation Cache ROI:** A standard application translating millions of standard HR terms at this volume would generate a translation bill exceeding $1,000. Our custom "Synapse" cache suppresses this to approximately $80.
...
* **Elastic Search Provisioning:** The architecture is designed to transition from a "free" scale-to-zero model to a highly responsive, permanently warm enterprise search engine for just $15 a month by toggling a single configuration line.
