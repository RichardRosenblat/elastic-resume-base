...
> **Distributed tracing:** The `x-correlation-id` UUID generated by the frontend travels the full request chain. The BFF Gateway attaches it to every downstream call via `AsyncLocalStorage` + an Axios request interceptor in `httpClient.ts`. All services log the correlation ID in every structured log entry, enabling end-to-end request tracing in Cloud Logging. The frontend does **not** send `x-cloud-trace-context` — this is derived server-side from the correlation ID.
