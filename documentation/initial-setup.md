...
* **Environment Configuration:** All environment variables for every service are stored in a single **`env.yaml`** file at the repository root. This file ships with safe defaults for local development (Firebase emulator addresses, demo project IDs, etc.) and is committed to version control. Fill in any sensitive values (e.g. `GOOGLE_SERVICE_ACCOUNT_KEY`) locally and never commit them. For local secret overrides, create a git-ignored `docker-compose.override.yml`. See [Docker Orchestration](docker-orchestration.md) for details.
