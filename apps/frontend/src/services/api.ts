...
apiClient.interceptors.request.use(async (reqConfig) => {
  // Generate a unique correlation ID for every outbound request so the backend
  // can trace it end-to-end without emitting a "header absent" warning.
  // Only x-correlation-id is set here; x-cloud-trace-context is an internal
  // server-side concern and must not be generated by the browser client.
